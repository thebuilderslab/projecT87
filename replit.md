...
- **Borrow/Withdraw Token Routing (Critical Fix Feb 2026):** Aave V3's `borrow(onBehalfOf=user)` and `withdraw()` send tokens to `msg.sender` (DelegationManager), not the user. **Contract source fix (pending redeployment):** `executeBorrow` and `executeWithdraw` in `REAADelegationManager.sol` now include `IERC20(asset).transfer(user, amount)` after the Aave call, making forwarding atomic. **TODO: Redeploy contract** at `0x7427370Ab4C311B090446544078c819b3946E59d` with the updated source. All users will need to re-approve the new contract address. **Temporary Python workaround (active until redeployment):** `delegated_borrow()` uses `_rescue_tokens_from_dm()` (emergencyWithdrawToken DM→bot) and keeps tokens in bot wallet for distribution. `delegated_withdraw()` uses same rescue + `_forward_tokens_to_user()` to deliver to user. Bot wallet must be DM owner for `emergencyWithdrawToken` access.
- **Distribution Token Flow (Bot-Wallet-Centric):** After borrow, DAI stays in bot wallet. Distribution steps operate from bot balance directly — no `pull_token_from_user` needed. `delegated_supply_*_onbehalf()` calls Aave Pool directly from bot with `onBehalfOf=user`, bypassing DM. Swaps (Uniswap) happen from bot wallet. Only transfers TO user (gas reserve DAI, USDC tax) use `_forward_tokens_to_user()`. This architecture avoids the problem that user only approved the DM contract, not the bot wallet directly.
