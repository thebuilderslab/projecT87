...
- **Notifications:** `notifications` table with `wallet_address` column for multi-tenant filtering. Functions: `create_notification()` (global), `add_notification(wallet_address, message, priority, title)` (per-wallet), `get_notifications_for_wallet(wallet_address, limit)`. Auto-triggers in `strategy_engine.py`: Growth/Capacity distribution complete, Macro/Micro short entry, Short close, Nurse sweep complete.
...
- **Concurrent Wallet Processing:** `run_autonomous_mainnet.py` uses native `asyncio.gather()` with `asyncio.Semaphore(5)` for concurrent wallet processing (max `MAX_CONCURRENT_WALLETS=5`). Each wallet runs via `asyncio.to_thread()` to offload blocking Web3 RPC calls. `RPC_DELAY_SECONDS=0.5` between wallet starts prevents HTTP 429 bans.
- **Transaction Broadcast Lock & Local Nonce Manager:** All bot-operator-signed transactions across `delegation_client.py`, `aave_integration.py`, and `uniswap_integration.py` are serialized through a single `threading.Lock()` (`_tx_broadcast_lock`) with local nonce tracking (`_local_nonce`). This prevents nonce collisions when concurrent `asyncio.to_thread()` workers attempt to sign from the same bot wallet. Logic: (1) Acquire lock, (2) if `_local_nonce is None`, fetch from RPC via `get_transaction_count('pending')`, else use cached value, (3) build/sign/send, (4) increment `_local_nonce += 1`, (5) release lock. On nonce errors ("nonce too low", "already known", "replacement transaction"), `_local_nonce` resets to `None` forcing a fresh RPC pull. Public API: `get_tx_broadcast_lock()`, `acquire_nonce()`, `confirm_nonce()`, `reset_nonce()` — used by `_sign_and_send()` methods in aave_integration and uniswap_integration. Central paths: `_send_bot_tx()` (contract calls) and `_send_bot_raw_tx()` (raw ETH transfers) in delegation_client.py.
...
- **FastAPI + Flask Hybrid Server:** `api_server.py` runs FastAPI (Uvicorn) on port 5000. Flask dashboard from `web_dashboard.py` is mounted at root `/` via `WSGIMiddleware` (after all `/api/v1/` routes). `run_both.py` launches `api_server.py` instead of `web_dashboard.py`. Swagger docs at `/docs`.
- **OpenClaw REST API Endpoints:** `GET /api/v1/health` (unauthenticated status check), `GET /api/v1/vault/health` (authenticated, returns Aave position for user wallet), `POST /api/v1/credit/borrow` (authenticated, triggers capacity/growth distribution via strategy engine), `GET /api/v1/notifications` (authenticated, returns recent notifications for user wallet). All authenticated endpoints use `X-API-Key` header validated against `api_keys` table via `db.validate_api_key()`.
- **Black Box Pydantic Schemas:** `BorrowRequest` only accepts `amount: float` and `asset: str`. No risk parameters (health factor, collateral thresholds) can be passed — the platform enforces its own hardcoded risk rules.
- **Event Loop Safety:** All FastAPI endpoints that call synchronous Web3/strategy code use `def` (not `async def`), so FastAPI automatically runs them in a background threadpool without blocking the event loop.
- **Dashboard Activity Feed:** `GET /api/user/activity` (Flask, authenticated via X-Auth-Token) returns recent notifications for the logged-in user. Used by the frontend dashboard to display strategy action history.
