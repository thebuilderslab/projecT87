...
- **Borrow/Withdraw Token Routing (Critical Fix Feb 2026):** Aave V3's `borrow(onBehalfOf=user)` and `withdraw()` send tokens to `msg.sender` (DelegationManager), not the user. **Contract source fix (pending redeployment):** `executeBorrowAndTransfer` in `REAADelegationManager.sol` atomically borrows and transfers to user in a single call. **Feature flag:** `USE_ATOMIC_BORROW` in `delegation_client.py` — set `True` after contract redeployment. **Current 3-step workaround (USE_ATOMIC_BORROW=False):** `delegated_borrow()` calls `executeBorrow` (Aave→DM), then `_rescue_tokens_from_dm` (DM→BOT via emergencyWithdrawToken), then `_forward_tokens_to_user` (BOT→USER). Result: tokens always land in USER wallet.
- **Distribution Token Flow (User-Custodial, Feb 2026):** After borrow, tokens land in USER wallet (never bot). Distribution steps 2-7 each call `pull_token_from_user(wallet, token, amount)` (ERC20 `transferFrom`) to pull tokens from user to bot for swaps/supplies. **Dual approval model:** User must approve BOTH (1) DM contract for Aave primitives AND (2) BOT wallet for ERC20 token pulls. Frontend grants both at wallet connection time. Step 5 (gas reserve) requires no pull — DAI stays in user wallet. Swaps happen in BOT wallet, final outputs (USDC, supply txs) go to user.
