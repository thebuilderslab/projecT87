...
Using DAI flash loan with SELL direction (DAI→ARB) instead of BUY (ARB→DAI)
"""
...
def execute_final_debt_swap():
    """Execute debt swap using DAI flash loan with correct SELL direction"""
    
    print("🚀 FINAL DEBT SWAP EXECUTION")
    print("=" * 60)
    print("Using DAI flash loan with SELL direction (DAI→ARB)")
    print("=" * 60)
    ...
    try:
        ...
        # Step 2: Get ParaSwap route for BUY direction (ARB→DAI)
        print(f"\n🔄 STEP 2: Get ParaSwap Route (BUY ARB→DAI)")
        
        # Use BUY direction: exact amount of DAI needed for repayment
        swap_amount = 5.0
        swap_amount_wei = agent.w3.to_wei(swap_amount, 'ether')
        
        # BUY direction: specific amount of DAI out, get ARB cost
        # Add small buffer for flash loan premium
        repay_amount_with_buffer = agent.w3.to_wei(swap_amount * 1.005, 'ether')  # 0.5% buffer
        
        price_params = {
            'srcToken': '0x912CE59144191C1204E64559FE8253a0e49E6548',  # ARB (what adapter will have)
            'destToken': '0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1',  # DAI (what adapter needs)
            'amount': str(repay_amount_with_buffer),  # Exact amount of DAI to receive
            'srcDecimals': 18,
            'destDecimals': 18,
        ...
        print(f"   ✅ Route: Need {arb_needed:.6f} ARB → Get {swap_amount * 1.005:.3f} DAI")
        
        # Get ParaSwap transaction calldata for SELL direction
        ...
        print(f"   ✅ ParaSwap calldata obtained for BUY direction")
        
        # Step 3: Create swapDebt transaction with corrected parameters
        print(f"\n🚀 STEP 3: Execute Debt Swap (DAI Flash Loan)")
        ...
        # Create DebtSwapParams with corrected flow:
        # - Flash loan DAI (debtAsset) 
        # - Use that DAI to repay debt
        # - Borrow ARB (newDebtAsset)
        # - Swap ARB back to DAI to repay flash loan
        debt_swap_params = (
            '0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1',  # debtAsset (DAI - will be flash loaned)
            swap_amount_wei,                                 # debtRepayAmount (5 DAI)
            2,                                               # debtRateMode (2 = variable)
            '0x912CE59144191C1204E64559FE8253a0e49E6548',  # newDebtAsset (ARB - what we'll owe)
            src_amount + int(src_amount * 0.05),             # maxNewDebtAmount (ARB needed + 5% slippage)
            '0x0000000000000000000000000000000000000000',  # extraCollateralAsset (none)
            0,                                               # extraCollateralAmount (0)
            paraswap_data                                    # paraswapData (BUY ARB→DAI)
        )
        
        # Empty credit delegation permit (already approved)
        ...
        # Build transaction
        print(f"   Building swapDebt transaction with DAI flash loan...")
        ...
        print(f"      Flow: Flash loan DAI → Repay DAI debt → Borrow ARB → Swap ARB→DAI")
        ...
        if receipt['status'] == 1:
            ...
            print(f"   Debt swap amount: {dai_debt - new_dai_debt:.6f} DAI → {arb_debt:.6f} ARB")
...
if __name__ == "__main__":
    success = execute_final_debt_swap()
    
    if success:
        print(f"\n🎉 DEBT SWAP BREAKTHROUGH!")
        print(f"Successfully swapped DAI debt for ARB debt using corrected flow")
        print(f"Production-ready debt swap system now functional!")
    else:
        print(f"\n❌ DEBT SWAP STILL NEEDS DEBUGGING")
        print(f"Check the error details and gas usage for clues")